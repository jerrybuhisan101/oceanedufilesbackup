...
<div id="rpwwt-<?php echo $args[ 'widget_id' ];?>" class="rpwwt-widget">
	...
	<?php while ( $r->have_posts() ) : $r->the_post(); ?>
		<li<?php 
			...
			if ( ! $bools[ 'hide_title' ] ) {
				?><span class="rpwwt-post-title"><?php if ( $post_title = $this->get_the_trimmed_post_title( $ints[ 'post_title_length' ] ) ) { echo $post_title; } else { the_ID(); } ?></span><?php
			}
			?></a><?php 
			...
			if ( $bools[ 'show_excerpt' ] ) : 
				?><div class="rpwwt-post-excerpt"><?php echo $this->get_the_trimmed_excerpt( $ints[ 'excerpt_length' ], $excerpt_more, $bools[ 'ignore_excerpt' ], $bools[ 'set_more_as_link' ], $link_target ); ?></div><?php 
			endif;
