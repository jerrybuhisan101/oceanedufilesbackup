...
class Recent_Posts_Widget_With_Thumbnails extends WP_Widget {
	...
	function __construct() {
		...
		$this->defaults[ 'excerpt_length' ]		= absint( apply_filters( 'rpwwt_excerpt_length', 55 ) ); // number of chars of excerpt
		$this->defaults[ 'excerpt_more' ]		= apply_filters( 'rpwwt_excerpt_more', ' ' . '[&hellip;]' ); // characters to indicate further text
		$this->defaults[ 'number_posts' ]		= 5; // number of posts to show in the widget
		$this->defaults[ 'post_title_length' ] 	= 1000; // number of chars of excerpt
		...
		$this->plugin_version					= '5.3';
		$this->valid_excerpt_sources			= array( 'post_content', 'excerpt_field' );		$widget_ops 							= array( 'classname' => $this->plugin_slug, 'description' => $widget_desc );
	...
	function widget( $args, $instance ) {
		...
		$default_url 			= ( ! empty( $instance[ 'default_url' ] ) )			? esc_url_raw( $instance[ 'default_url' ] )				: $this->defaults[ 'thumb_url' ];
		...
		$this->current_thumb_dimensions = array( $ints[ 'thumb_width' ], $ints[ 'thumb_height' ] );
	...
	function update( $new_widget_settings, $old_widget_settings ) {
		...
		foreach ( $this->bools_true as $key ) {
			$instance[ $key ] = ( isset( $new_widget_settings[ $key ] ) ) ? (bool) $new_widget_settings[ $key ] : true;
		}
	...
	function form( $instance ) {
		...
		$default_url			= ( isset( $instance[ 'default_url' ] ) )			? esc_url_raw( $instance[ 'default_url' ] )				: $this->defaults[ 'thumb_url' ];
		$category_ids			= ( isset( $instance[ 'category_ids' ] ) )			? array_map( 'absint', $instance[ 'category_ids' ] )	: $this->defaults[ 'category_ids' ];
		// initialize integer variables
		$ints = array();
		foreach ( $this->ints as $key ) {
		...
		// sanitize vars
		if ( ! $category_ids )		$category_ids		= $this->defaults[ 'category_ids' ];
		if ( ! $default_url )		$default_url		= $this->defaults[ 'thumb_url' ];
		if ( ! $thumb_dimensions )	$thumb_dimensions	= $this->defaults[ 'thumb_dimensions' ];
	...
	private function make_css_file () {
		...
		foreach ( $all_instances as $number => $settings ) {
			...
			if ( $bools[ 'keep_aspect_ratio' ] ) {
				$css_code .= sprintf( '#rpwwt-%s-%d img { max-width: %dpx; width: 100%%; height: auto; }', $this->plugin_slug, $number, $ints[ 'thumb_width' ] );
				$css_code .= "\n"; 
			} else {
				$css_code .= sprintf( '#rpwwt-%s-%d img { width: %dpx; height: %dpx; }', $this->plugin_slug, $number, $ints[ 'thumb_width' ], $ints[ 'thumb_height' ] );
				$css_code .= "\n"; 
			}
